require([
  ...
  const view = new MapView({
    container: "viewDiv",
    map: webMap //WebMap is incorect. The variable name is webMap
  });

  const homeBtn = new Home({ //missing new key word!
    view: view
  })
  ...
  view.ui.add("layerList-btn", "top-right"); // layerList - btn is incorrect. It must be with quotes and without white space;
  view.ui.add("basemap-btn", "top-right"); //basemap - btn is incorrect. It must be with quotes and without white space;

  document
    .getElementById("layerList-btn")
    .addEventListener("click", function () {
      toggleButton("layerList") //layerLiat - syntax error
    })
  ...
  function toggleButton(item) {
    ...
    if (item == "layerlist") { // layerlist ...list must be upper case
